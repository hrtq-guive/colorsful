...
const ColorPickerVideoCard = ({ video, index }) => {
    ...
    // Asset path
    const jpgPath = `/captures/${videoId}_45s.jpg`;

    // Load saved picked color on mount
    useEffect(() => {
        const savedColor = getPickedColor(video.url);
        if (savedColor) {
    ...
    const handleClick = () => {
        if (hoveredColor) {
            ...
            saveColorCategory(video.url, '', hoveredColor);
        }
    };
                ...
                >
                    <div
                        style={{
                            width: '100%',
                            height: '36px',
                            backgroundColor: video.hex45 || video.color,
                            borderRadius: '3px',
...
const GridSetup = () => {
    ...
    // Filter to show only videos with GIFs (same as Grid.jsx)
    ...
    const sortedVideos = useMemo(() => {
        ...
        const videosWithColors = videosWithGifs.map(video => {
            const rgb = hexToRgb(video.hex45 || video.color);
        ...
        // Post-processing: Very aggressive multi-pass outlier correction
        // Run 5 passes to catch all misplaced colors
        for (let pass = 0; pass < 5; pass++) {
            for (let i = 5; i < sorted.length - 5; i++) {
                ...
                // Calculate average hue of neighbors
                let avgHue = 0;
                neighbors.forEach(n => avgHue += n.hsl.h);
                avgHue /= neighbors.length;

                // Check if current video's hue is significantly different
                let hueDiff = Math.abs(current.hsl.h - avgHue);
                if (hueDiff > 180) hueDiff = 360 - hueDiff;

                // Very strict threshold: 15° and saturation > 0.15
                if (hueDiff > 15 && current.hsl.s > 0.15) {
                    // Find best position in sorted array near that hue
                    let bestPos = i;
                    let minHueDiff = hueDiff;

                    for (let j = 0; j < sorted.length; j++) {
                        if (Math.abs(j - i) < 10) continue; // Skip nearby positions

                        let diff = Math.abs(sorted[j].hsl.h - current.hsl.h);
                    ...
                    // Relocate if we found a better position (30% improvement)
                    if (bestPos !== i && minHueDiff < hueDiff * 0.3) {
                    ...
                    <div style={{
                        ...
                        <div>
                            ...
                            <div style={{
                                ...
                                Hover to preview • Click to save • {stats.totalEdits} colors picked
                            </div>
                        </div>
